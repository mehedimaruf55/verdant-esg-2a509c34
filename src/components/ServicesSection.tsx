...
const ServicesSection = () => {
  return (
    <section id="services" className="py-32 lg:py-40 bg-brand-grey-light">
      <div className="mx-auto max-w-7xl px-6">
        ...
        <div className="mt-16">
          {services.map((service, i) => (
            <motion.div
              key={service.num}
              className="group border-t border-brand-grey/30 last:border-b py-8 lg:py-10 grid lg:grid-cols-12 gap-4 lg:gap-8 items-center cursor-pointer"
              initial={{ opacity: 0, y: 15 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true, margin: "-50px" }}
              transition={{ duration: 0.5, delay: i * 0.08 }}
            >
              <span className="lg:col-span-1 text-xs text-brand-grey font-mono">{service.num}</span>
              ...
              <div className="lg:col-span-1 flex justify-end">
                <div className="w-10 h-10 rounded-full border border-brand-grey/30 flex items-center justify-center text-brand-grey group-hover:border-brand-green-dark group-hover:bg-brand-green-dark group-hover:text-brand-white transition-all duration-300">
